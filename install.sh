...
# builds the `bloodcandy` binary from the `rust/` workspace, and runs a
...
${COLOR_DIM}---------------${COLOR_RESET}
  ...
  ${COLOR_BOLD}6. 'bloodcandy' not found after install${COLOR_RESET}
     The binary lives at:
       rust/target/${BUILD_PROFILE}/bloodcandy
...
step "Building the bloodcandy workspace (${BUILD_PROFILE})"
...
CLAW_BIN="${RUST_DIR}/target/${BUILD_PROFILE}/bloodcandy"
...
if [ "${SKIP_VERIFY}" = "1" ]; then
    ...
    info "running: bloodcandy --version"
    if VERSION_OUT="$("${CLAW_BIN}" --version 2>&1)"; then
        ok "bloodcandy --version -> ${VERSION_OUT}"
    else
        error "bloodcandy --version failed:"
        printf '%s\n' "${VERSION_OUT}" 1>&2
        exit 1
    fi

    info "running: bloodcandy --help (smoke test)"
    if "${CLAW_BIN}" --help >/dev/null 2>&1; then
        ok "bloodcandy --help responded"
    else
        error "bloodcandy --help failed"
        exit 1
    fi
fi
