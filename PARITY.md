# Parity Status — blood-candy Rust Port
...
- Mock parity harness stats: **10 scripted scenarios**, **19 captured `/v1/messages` requests** in `rust/crates/blood-candy-cli/tests/mock_parity_harness.rs`.

## Mock parity harness — milestone 1

- [x] Deterministic Anthropic-compatible mock service (`rust/crates/mock-anthropic-service`)
- [x] Reproducible clean-environment CLI harness (`rust/crates/blood-candy-cli/tests/mock_parity_harness.rs`)
...
- **Why it matters:** `.github/workflows/rust-ci.yml` runs `cargo fmt --all --check` and `cargo test -p blood-candy-cli`; this lane removed a CI-specific sandbox assumption from runtime behavior.
