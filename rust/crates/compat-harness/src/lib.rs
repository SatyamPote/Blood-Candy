...
fn upstream_repo_candidates(primary_repo_root: &Path) -> Vec<PathBuf> {
    ...
    for ancestor in primary_repo_root.ancestors().take(4) {
        candidates.push(ancestor.join("blood-candy"));
        candidates.push(ancestor.join("clawd-code"));
    }

    candidates.push(primary_repo_root.join("reference-source").join("blood-candy"));
    candidates.push(primary_repo_root.join("vendor").join("blood-candy"));
