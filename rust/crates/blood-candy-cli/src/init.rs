...
const GITIGNORE_ENTRIES: [&str; 3] = [".bloodcandy/settings.local.json", ".bloodcandy/sessions/", ".clawhip/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let claw_dir = cwd.join(".bloodcandy");
    artifacts.push(InitArtifact {
        name: ".bloodcandy/",
        status: ensure_dir(&claw_dir)?,
    });

    let claw_json = cwd.join(".bloodcandy.json");
    artifacts.push(InitArtifact {
        name: ".bloodcandy.json",
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to Claw Code (BloodCandy.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.bloodcandy.json`; reserve `.bloodcandy/settings.local.json` for machine-local overrides.".to_string());
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".bloodcandy/"));
        assert!(rendered.contains(".bloodcandy.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".bloodcandy").is_dir());
        assert!(root.join(".bloodcandy.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".bloodcandy.json")).expect("read bloodcandy json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".bloodcandy/settings.local.json"));
        assert!(gitignore.contains(".bloodcandy/sessions/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join(".gitignore"), ".bloodcandy/settings.local.json\n").expect("write gitignore");
        ...
        assert!(second_rendered.contains(".bloodcandy/"));
        assert!(second_rendered.contains(".bloodcandy.json"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".bloodcandy/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".bloodcandy/sessions/").count(), 1);
