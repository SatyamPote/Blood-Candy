...
fn max_tokens_for_model(model: &str) -> u32 {
    if model.contains("opus") {
        32_000
    } else if model.contains('/') || std::env::var_os("OPENAI_BASE_URL").is_some() {
        // OpenAI-compat providers (Groq, Ollama, etc.) have strict limits.
        // Use 2048 as a safe default so we never exceed provider caps.
        2_048
    } else {
        64_000
    }
...
const VERSION: &str = "2.0.0-gold";
const BUILD_TARGET: Option<&str> = option_env!("TARGET");
const GIT_SHA: Option<&str> = option_env!("GIT_SHA");
const INTERNAL_PROGRESS_HEARTBEAT_INTERVAL: Duration = Duration::from_millis(500); // 2x faster heartbeat for v2.0
const POST_TOOL_STALL_TIMEOUT: Duration = Duration::from_secs(15);
const PRIMARY_SESSION_EXTENSION: &str = "jsonl";
const LEGACY_SESSION_EXTENSION: &str = "json";
const OFFICIAL_REPO_URL: &str = "https://github.com/SatyamPote/Blood-Candy";
const OFFICIAL_REPO_SLUG: &str = "SatyamPote/Blood-Candy";
const DEPRECATED_INSTALL_COMMAND: &str = "cargo install blood-candy";
...
fn main() {
    if let Err(error) = run() {
        ...
        } else if message.contains("`bloodcandy --help`") {
            eprintln!("error: {message}");
        } else {
            eprintln!(
                "error: {message}

Run `bloodcandy --help` for usage."
            );
        }
        std::process::exit(1);
    }
    ...
    while index < args.len() {
        match args[index].as_str() {
                    ...
                    ) =>
            {
                ...
                // the arg to the API (e.g. `bloodcandy prompt --help`) should show
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`bloodcandy {command_name}` is a slash command. Use `bloodcandy --resume SESSION.jsonl /{command_name}` or start `bloodcandy` and run `/{command_name}`."
        )
    } else {
        format!(
            "`bloodcandy {command_name}` is a slash command. Start `bloodcandy` and run `/{command_name}` inside the REPL."
        )
    };
    Some(guidance)
}
...
fn removed_auth_surface_error(command_name: &str) -> String {
    format!(
        "`bloodcandy {command_name}` has been removed. Set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN instead."
    )
}
...
fn parse_acp_args(args: &[String], output_format: CliOutputFormat) -> Result<CliAction, String> {
    match args {
        ...
        _ => Err(String::from(
            "unsupported ACP invocation. Use `bloodcandy acp`, `bloodcandy acp serve`, `bloodcandy --acp`, or `bloodcandy -acp`.",
        )),
    }
}
...
fn parse_direct_slash_cli_action(
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `bloodcandy` and run it there, or use `bloodcandy --resume SESSION.jsonl {command_name}` / `bloodcandy --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `bloodcandy --help` for CLI usage, or start `bloodcandy` and use /help.");
    message
}
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-claudecode:")
        .then_some(
            "Compatibility note: `/oh-my-claudecode:*` is a Claude Code/OMC plugin command. `bloodcandy` does not yet load plugin slash commands, Claude statusline stdin, or OMC session hooks.",
        )
}
...
/// Starts a minimal Model Context Protocol server that exposes bloodcandy's
...
/// Read `.bloodcandy/worker-state.json` from the current working directory and print it.
...
fn run_worker_state(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let state_path = cwd.join(".bloodcandy").join("worker-state.json");
...
fn run_mcp_serve() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let spec = McpServerSpec {
        server_name: "bloodcandy".to_string(),
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    match load_oauth_credentials() {
        ...
        .with_details(vec![
            ...
            "Suggested action  set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN; `bloodcandy login` is removed"
                .to_string(),
        ])
...
fn check_install_source_health() -> DiagnosticCheck {
    ...
    .with_details(vec![
        ...
        "Recommended path  build from this repo or use the upstream binary documented in README.md"
            .to_string(),
        format!(
            "Deprecated crate  `{DEPRECATED_INSTALL_COMMAND}` installs a deprecated stub and does not provide the `bloodcandy` binary"
        )
            .to_string(),
    ])
    .with_data(Map::from_iter([
...
const DUMP_MANIFESTS_OVERRIDE_HINT: &str =
    "Hint: set CLAUDE_CODE_UPSTREAM=/path/to/upstream or pass `bloodcandy dump-manifests --manifests-dir /path/to/upstream`.";
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .bloodcandy/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
    ...
    match command {
        ...
        SlashCommand::Clear { confirm } => {
            ...
            Ok(ResumeCommandOutcome {
                session: cleared,
                message: Some(format!(
                    "Session cleared\n  Mode             resumed session reset\n  Previous session {previous_session_id}\n  Backup           {}\n  Resume previous  bloodcandy --resume {}\n  New session      {new_session_id}\n  Session file     {}",
                    backup_path.display(),
                    backup_path.display(),
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                return Err(
                    "resumed /skills invocations are interactive-only; start `bloodcandy` and run `/skills <skill>` in the REPL".into(),
                );
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
...
fn enforce_broad_cwd_policy(
    ...
    if is_interactive {
        ...
        eprintln!(
            "Warning: bloodcandy is running from a very broad directory ({}).\n\
             The agent can read and search everything under this path.\n\
             Consider running from inside your project: cd /path/to/project && bloodcandy",
            cwd.display()
        );
        ...
        let message = format!(
            "bloodcandy is running from a very broad directory ({}). \
             The agent can read and search everything under this path. \
             Use --allow-broad-cwd to proceed anyway, \
             or run from inside your project: cd /path/to/project && bloodcandy",
            cwd.display()
        );
        match output_format {
            CliOutputFormat::Json => {
...
impl LiveCli {
    ...
    fn print_mcp(
        ...
        // `bloodcandy mcp serve` starts a stdio MCP server exposing bloodcandy's built-in
...
fn render_repl_help() -> String {
    [
        ...
        "  Auto-save            .bloodcandy/sessions/<session-id>.jsonl".to_string(),
    ...
    json!({
        ...
        "workspace": {
            ...
            "session_id": context.session_path.as_ref().and_then(|path| {
                ...
                // .bloodcandy/sessions/. Extract the stem (drop the .jsonl extension).
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Usage            bloodcandy status
  Purpose          show the local workspace snapshot without entering the REPL
  Output           model, permissions, git state, config files, and sandbox status
  Related          /status · bloodcandy --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            bloodcandy sandbox
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Related          /sandbox · bloodcandy status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            bloodcandy doctor
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Related          /doctor · bloodcandy --resume latest /doctor"
            .to_string(),
        LocalHelpTopic::Acp => "ACP / Zed
  Usage            bloodcandy acp [serve]
  Aliases          bloodcandy --acp · bloodcandy -acp
  ...
  Related          ROADMAP #64a (discoverability) · ROADMAP #76 (real ACP support) · bloodcandy --help"
            .to_string(),
    }
}
...
fn print_acp_status(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let message = "ACP/Zed editor integration is not implemented in blood-candy yet. `bloodcandy acp serve` is only a discoverability alias today; it does not launch a daemon or Zed-specific protocol endpoint. Use the normal terminal surfaces for now and track ROADMAP #76 for real ACP support.";
    match output_format {
        CliOutputFormat::Text => {
            println!(
                "ACP / Zed\n  Status           discoverability only\n  Launch           `bloodcandy acp serve` / `bloodcandy --acp` / `bloodcandy -acp` report status only; no editor daemon is available yet\n  Today            use `bloodcandy prompt`, the REPL, or `bloodcandy doctor` for local verification\n  Tracking         ROADMAP #76\n  Message          {message}"
            );
        }
        CliOutputFormat::Json => {
            println!(
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        // (`crates/blood-candy-cli/tests/compact_output.rs`) to point
        // bloodcandy at its fake Anthropic endpoint. We also attach a
...
fn format_context_window_blocked_error(session_id: &str, error: &api::ApiError) -> String {
    ...
    lines.push(format!(
        "  Resume compact   bloodcandy --resume {session_id} /compact"
    ));
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "bloodcandy v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(
        out,
        "  bloodcandy [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  bloodcandy [--model MODEL] [--output-format text|json] prompt TEXT"
    )?;
    writeln!(out, "      Send one prompt and exit")?;
    writeln!(
        out,
        "  bloodcandy [--model MODEL] [--output-format text|json] TEXT"
    )?;
    writeln!(out, "      Shorthand non-interactive prompt mode")?;
    writeln!(
        out,
        "  bloodcandy --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  bloodcandy help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  bloodcandy version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  bloodcandy status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  bloodcandy sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  bloodcandy doctor")?;
    writeln!(
        out,
        "      Diagnose local auth, config, workspace, and sandbox health"
    )?;
    writeln!(out, "  bloodcandy acp [serve]")?;
    writeln!(
        out,
    ...
    writeln!(out, "  bloodcandy dump-manifests [--manifests-dir PATH]")?;
    writeln!(out, "  bloodcandy bootstrap-plan")?;
    writeln!(out, "  bloodcandy agents")?;
    writeln!(out, "  bloodcandy mcp")?;
    writeln!(out, "  bloodcandy skills")?;
    writeln!(out, "  bloodcandy system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(out, "  bloodcandy init")?;
    writeln!(
        out,
        "  bloodcandy export [PATH] [--session SESSION] [--output PATH]"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .bloodcandy/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  bloodcandy --model claude-opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  bloodcandy --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(out, "  bloodcandy --compact \"summarize Cargo.toml\" | wc -l")?;
    writeln!(
        out,
        "  bloodcandy --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  bloodcandy --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  bloodcandy --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  bloodcandy agents")?;
    writeln!(out, "  bloodcandy mcp show my-server")?;
    writeln!(out, "  bloodcandy /skills")?;
    writeln!(out, "  bloodcandy doctor")?;
    ...
    writeln!(out, "  bloodcandy init")?;
    writeln!(out, "  bloodcandy export")?;
    writeln!(out, "  bloodcandy export conversation.md")?;
    Ok(())
}
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   bloodcandy --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("blood-candy-cli-{nanos}-{unique}"))
    }
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".bloodcandy")).expect("project config dir should exist");
        std::fs::create_dir_all(&config_home).expect("config home should exist");
        std::fs::write(
            cwd.join(".bloodcandy").join("settings.json"),
    ...
    fn parses_direct_agents_mcp_and_skills_slash_commands() {
        ...
        assert!(error.contains("bloodcandy --resume SESSION.jsonl /status"));
    }
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("bloodcandy --help"));
    }
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("Auto-save            .bloodcandy/sessions/<session-id>.jsonl"));
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("bloodcandy help"));
        assert!(help.contains("bloodcandy version"));
        assert!(help.contains("bloodcandy status"));
        assert!(help.contains("bloodcandy sandbox"));
        assert!(help.contains("bloodcandy init"));
        assert!(help.contains("bloodcandy acp [serve]"));
        assert!(help.contains("bloodcandy agents"));
        assert!(help.contains("bloodcandy mcp"));
        assert!(help.contains("bloodcandy skills"));
        assert!(help.contains("bloodcandy /skills"));
        assert!(help.contains("SatyamPote/Blood-Candy"));
        assert!(help.contains("cargo install blood-candy"));
        assert!(!help.contains("bloodcandy login"));
        assert!(!help.contains("bloodcandy logout"));
    }
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("bloodcandy --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("bloodcandy --resume latest"));
        assert!(help.contains("bloodcandy --resume latest /status /diff /export notes.txt"));
    }
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".bloodcandy/sessions/legacy.json");
    ...
    fn load_session_reference_rejects_workspace_mismatch() {
        ...
        let session_path = workspace_a.join(".bloodcandy/sessions/legacy-cross.jsonl");
        std::fs::create_dir_all(
            session_path
                .parent()
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".bloodcandy/sessions/<session-id>.jsonl"));
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("bloodcandy-cli-{label}-{nanos}"))
    }
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/blood-candy-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p blood-candy-cli"}"#)
                .contains("cargo test -p blood-candy-cli")
        );
        assert_eq!(
