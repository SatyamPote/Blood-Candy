...
fn status_command_applies_model_and_permission_mode_flags() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_claw"))
        ...
        .output()
        .expect("bloodcandy should launch");
...
fn config_command_loads_defaults_from_standard_config_locations() {
    ...
    let config_home = temp_dir.join("home").join(".bloodcandy");
    fs::create_dir_all(temp_dir.join(".bloodcandy")).expect("project config dir should exist");
    ...
    fs::write(temp_dir.join(".bloodcandy.json"), r#"{"model":"sonnet"}"#)
        .expect("write project settings");
    fs::write(
        temp_dir.join(".bloodcandy").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    assert!(stdout.contains(temp_dir.join(".bloodcandy.json").to_str().expect("utf8 path")));
    assert!(stdout.contains(
        temp_dir
            .join(".bloodcandy")
...
fn doctor_command_runs_as_a_local_shell_entrypoint() {
    ...
    let output = command_in(&temp_dir)
        ...
        .arg("doctor")
        .output()
        .expect("bloodcandy doctor should launch");
...
fn local_subcommand_help_does_not_fall_through_to_runtime_or_provider_calls() {
    ...
    assert!(doctor_stdout.contains("Usage            bloodcandy doctor"));
    ...
    assert!(status_stdout.contains("Usage            bloodcandy status"));
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "bloodcandy-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
