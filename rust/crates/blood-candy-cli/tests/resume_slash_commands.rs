...
fn resumed_binary_accepts_slash_commands_with_arguments() {
    ...
    assert!(stdout.contains("Resume previous  bloodcandy --resume"));
...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    fs::create_dir_all(project_dir.join(".bloodcandy")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".bloodcandy").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
...
fn run_claw_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
    ...
    command.output().expect("bloodcandy should launch")
}
