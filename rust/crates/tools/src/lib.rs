...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    push_skill_lookup_root(
        &mut roots,
        std::path::PathBuf::from("/home/bellman/.bloodcandy/skills"),
        SkillLookupOrigin::SkillsDir,
    );
    push_skill_lookup_root(
        &mut roots,
...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".bloodcandy"));
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".bloodcandy"));
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "clawguide" | "clawguideagent" | "guide" => String::from("bloodcandy-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".bloodcandy").join("settings.local.json"),
    })
}
...
fn config_home_dir() -> Result<PathBuf, String> {
    ...
    Ok(PathBuf::from(home).join(".bloodcandy"))
}
...
mod tests {
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        use std::fs;
        // Write a .bloodcandy/settings.json in a temp dir with trustedRoots
        let worktree = temp_path("config-trust-worktree");
        let claw_dir = worktree.join(".bloodcandy");
        fs::create_dir_all(&claw_dir).expect("create .bloodcandy dir");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .bloodcandy/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".bloodcandy").join("worker-state.json");
    ...
    fn skill_loads_local_skill_prompt() {
        ...
        fs::write(
            ...
            "# help\n\nGuide on using blood-candy-engine plugin\n",
        )
        ...
        assert!(output["prompt"]
            .as_str()
            .expect("prompt")
            .contains("Guide on using blood-candy-engine plugin"));
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".bloodcandy").join("skills").join("plan");
        let command_dir = root.join(".bloodcandy").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".bloodcandy/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".bloodcandy/commands/handoff.md"));
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".bloodcandy")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".bloodcandy")).expect("cwd dir");
        std::fs::write(
            home.join(".bloodcandy").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".bloodcandy")).expect("cwd dir");
        std::fs::write(
            cwd.join(".bloodcandy").join("settings.local.json"),
        ...
        let local_settings = std::fs::read_to_string(cwd.join(".bloodcandy").join("settings.local.json"))
            .expect("local settings after enter");
        assert!(local_settings.contains(r#""defaultMode": "plan""#));
        let state =
            std::fs::read_to_string(cwd.join(".bloodcandy").join("tool-state").join("plan-mode.json"))
...
"#,
    ...
    fn run_task_packet_creates_packet_backed_task() {
        ...
        let result = run_task_packet(TaskPacket {
            ...
            repo: "blood-candy-parity".to_string(),
        ...
        assert_eq!(output["task_packet"]["repo"], "blood-candy-parity");
        assert_eq!(
