...
pub struct LaneOwnership {
    ...
    pub owner: String,
    /// Workflow scope (e.g., blood-candy-dogfood, external-git-maintenance)
...
mod tests {
    ...
    fn lane_ownership_binding_includes_workflow_scope() {
        let ownership = LaneOwnership {
            owner: "bloodcandy-1".to_string(),
            workflow_scope: "blood-candy-dogfood".to_string(),
            watcher_action: WatcherAction::Act,
        };

        assert_eq!(ownership.owner, "bloodcandy-1");
        assert_eq!(ownership.workflow_scope, "blood-candy-dogfood");
