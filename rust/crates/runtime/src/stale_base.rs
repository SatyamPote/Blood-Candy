...
/// Read the `.bloodcandy-base` file from the given directory and return the trimmed
/// commit hash, or `None` when the file is absent or empty.
pub fn read_claw_base_file(cwd: &Path) -> Option<String> {
    let path = cwd.join(".bloodcandy-base");
...
/// fall back to reading `.bloodcandy-base` from `cwd`.
...
mod tests {
    ...
    fn reads_claw_base_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".bloodcandy-base"), "abc1234def5678\n").expect("write .bloodcandy-base");
    ...
    fn returns_none_for_empty_claw_base_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".bloodcandy-base"), "  \n").expect("write empty .bloodcandy-base");
    ...
    fn resolve_expected_base_prefers_flag_over_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".bloodcandy-base"), "from_file\n").expect("write .bloodcandy-base");
    ...
    fn matches_with_claw_base_file_in_real_repo() {
        ...
        init_repo(&root);
        let sha = head_sha(&root);
        fs::write(root.join(".bloodcandy-base"), format!("{sha}\n")).expect("write .bloodcandy-base");
        let source = resolve_expected_base(None, &root);
    ...
    fn diverged_with_claw_base_file_after_new_commit() {
        ...
        init_repo(&root);
        let old_sha = head_sha(&root);
        fs::write(root.join(".bloodcandy-base"), format!("{old_sha}\n")).expect("write .bloodcandy-base");
