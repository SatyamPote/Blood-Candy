...
pub struct SessionStore {
    ...
    /// `/home/user/project/.bloodcandy/sessions/a1b2c3d4e5f60718/`.
...
impl SessionStore {
    ...
    ///
    /// The on-disk layout becomes `<cwd>/.bloodcandy/sessions/<workspace_hash>/`.
...
fn format_missing_session_reference(reference: &str) -> String {
    format!(
        "session not found: {reference}\nHint: managed sessions live in .bloodcandy/sessions/. Try `{LATEST_SESSION_REFERENCE}` for the most recent session or `/session list` in the REPL."
    )
}

fn format_no_managed_sessions() -> String {
    format!(
        "no managed sessions found in .bloodcandy/sessions/\nStart `bloodcandy` to create a session, then rerun with `--resume {LATEST_SESSION_REFERENCE}`."
    )
}
...
mod tests {
    ...
    fn session_store_rejects_legacy_session_from_other_workspace() {
        ...
        let store_b = SessionStore::from_cwd(&workspace_b).expect("store b should build");
        let legacy_root = workspace_b.join(".bloodcandy").join("sessions");
    ...
    fn session_store_loads_safe_legacy_session_from_same_workspace() {
        ...
        fs::create_dir_all(&base).expect("base dir should exist");
        let store = SessionStore::from_cwd(&base).expect("store should build");
        let legacy_root = base.join(".bloodcandy").join("sessions");
