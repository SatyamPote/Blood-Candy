...
/// Write current worker state to `.bloodcandy/worker-state.json` under the worker's cwd.
...
fn emit_state_file(worker: &Worker) {
    let state_dir = std::path::Path::new(&worker.cwd).join(".bloodcandy");
...
mod tests {
    ...
    fn emit_state_file_writes_worker_status_on_transition() {
        ...
        let state_path = cwd_path.join(".bloodcandy").join("worker-state.json");
        assert!(
            state_path.exists(),
    ...
    fn startup_timeout_classifies_trust_required_when_prompt_blocked() {
        ...
        let timed_out = registry
            .observe_startup_timeout(&worker.worker_id, "bloodcandy prompt", true, true)
            .expect("startup timeout observe should succeed");
