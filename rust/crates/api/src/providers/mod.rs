//! Provider implementation registry and routing logic.
//! v2.0.0 Refactor for Blood Candy.
...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    // If the model name starts with a prefix followed by a slash (e.g. "openrouter/...")
    // we derive the environment variable names from that prefix.
    if let Some((prefix, _)) = canonical.split_once('/') {
        if prefix != "claude" && prefix != "grok" {
            let env_prefix = prefix.to_uppercase();
            let auth_env = Box::leak(format!("{}_API_KEY", env_prefix).into_boxed_str());
            let base_url_env = Box::leak(format!("{}_BASE_URL", env_prefix).into_boxed_str());
            
            return Some(ProviderMetadata {
                provider: ProviderKind::OpenAi,
                auth_env,
                base_url_env,
                default_base_url: match prefix {
                    "openrouter" => "https://openrouter.ai/api/v1",
                    "gemini" => "https://generativelanguage.googleapis.com/v1beta/openai",
                    "groq" => "https://api.groq.com/openai/v1",
                    _ => openai_compat::DEFAULT_OPENAI_BASE_URL,
                },
            });
        }
    }
    ...
    model_token_limit(model).map_or_else(
        || {
            ...
            if canonical.contains("opus") {
                32_000
            // OpenAI-compat routed models (e.g. Groq, Ollama) often have
            // strict limits well below 64k. Default to a safe 8k to avoid
            // provider-side 400 errors for unknown models.
...
pub fn model_token_limit(model: &str) -> Option<ModelTokenLimit> {
    let canonical = resolve_model_alias(model);
    // Strip any provider routing prefix (e.g. "openai/", "groq/") so that
    // "openai/llama-3.3-70b-versatile" hits the same branch as
    // "llama-3.3-70b-versatile" in the match below.
    let bare: String = canonical
        .find('/')
        .map_or_else(|| canonical.clone(), |pos| canonical[pos + 1..].to_string());
    match bare.as_str() {
        ...
        // Groq-hosted free models (max_tokens hard cap = 32768)
        ...
        | "mixtral-8x7b-32768"
        | "gemma2-9b-it"
        | "gemma-7b-it" => Some(ModelTokenLimit {
            max_output_tokens: 8_192,
            context_window_tokens: 32_768,
        }),
        // Groq DeepSeek R1
        "deepseek-r1-distill-llama-70b" => Some(ModelTokenLimit {
            max_output_tokens: 8_192,
            context_window_tokens: 32_768,
        }),
        _ => None,
...
mod tests {
    ...
    fn qwen_prefix_routes_to_dashscope_not_anthropic() {
        // User request from Discord #BloodCandy-get-help: web3g wants to use
    ...
    fn plugin_config_max_output_tokens_overrides_model_default() {
        ...
        let home = root.join("home").join(".bloodcandy");
        std::fs::create_dir_all(cwd.join(".bloodcandy")).expect("project config dir");
