...
impl OpenAiCompatConfig {
    ...
    /// Requested via Discord #BloodCandy-get-help: native Alibaba API for
...
impl OpenAiCompatClient {
    ...
    pub fn new(api_key: impl Into<String>, config: OpenAiCompatConfig) -> Self {
        ...
        if config.provider_name.to_lowercase().contains("openrouter") {
            headers.insert("HTTP-Referer", "https://github.com/SatyamPote/Blood-Candy".parse().unwrap());
            headers.insert("X-Title", "Blood Candy".parse().unwrap());
        }
        ...
        Self {
            ...
            max_retries: 2, // Fewer retries for a snappier feel
    ...
    async fn send_with_retry(
        ...
        if let Some((prefix, canonical)) = request.model.split_once('/') {
            let prefix_lower = prefix.to_lowercase();
            let env_p = self.config.api_key_env.to_lowercase();
            let prov_p = self.config.provider_name.to_lowercase();
            
            if env_p.starts_with(&prefix_lower) || prov_p.starts_with(&prefix_lower) {
