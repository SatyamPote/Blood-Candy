...
impl ProviderClient {
    ...
    pub fn from_model_with_anthropic_auth(
        ...
        match providers::detect_provider_kind(&resolved_model) {
            ...
            ProviderKind::OpenAi => {
                // OpenAI-compatible providers (DashScope, Gemini, etc.) use the
                // same wire format but different credentials/endpoints.
                let config = match providers::metadata_for_model(&resolved_model) {
                    Some(meta) => {
                        let provider_name = if let Some((prefix, _)) = resolved_model.split_once('/') {
                            prefix.to_uppercase()
                        } else {
                            "Custom".to_string()
                        };
                    ...
                    None => OpenAiCompatConfig::openai(),
                };
