...
This milestone adds a deterministic Anthropic-compatible mock service plus a reproducible CLI harness for the Rust `bloodcandy` binary.

## Artifacts

- `crates/mock-anthropic-service/` — mock `/v1/messages` service
- `crates/blood-candy-cli/tests/mock_parity_harness.rs` — end-to-end clean-environment harness
