...
cd rust/
cargo run -p blood-candy-cli -- --help

# Build the workspace
cargo build --workspace

# Run the interactive REPL
cargo run -p blood-candy-cli -- --model claude-opus-4-6

# One-shot prompt
cargo run -p blood-candy-cli -- prompt "explain this codebase"

# JSON output for automation
cargo run -p blood-candy-cli -- --output-format json prompt "summarize src/main.rs"
```
...
- `crates/blood-candy-cli/tests/mock_parity_harness.rs` — clean-env CLI harness
...
| Config file hierarchy (`.bloodcandy.json` + merged config sections) | ✅ |
| Permission system | ✅ |
...
`bloodcandy acp` is a local discoverability surface for editor-first users: it reports the current ACP/Zed status without starting the runtime. As of April 16, 2026, blood-candy does **not** ship an ACP/Zed daemon entrypoint yet, and `bloodcandy acp serve` is only a status alias until the real protocol surface lands.
...
Notable bloodcandy-first surfaces now available directly in slash form:
...
See [`../USAGE.md`](../USAGE.md) for usage examples and run `cargo run -p blood-candy-cli -- --help` for the live canonical command list.
...
└── crates/
    ...
    ├── blood-candy-cli/   # Main CLI binary (`bloodcandy`)
...
- **blood-candy-cli** — REPL, one-shot prompt, direct CLI subcommands, streaming display, tool call rendering, CLI argument parsing
